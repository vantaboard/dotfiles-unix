...
- **Neovim / Telescope:** `telescope-fzf-native` is skipped on Termux (requires a native `libfzf.so` build). Telescope still works with the built-in sorter; install `cmake` and build manually on Linux only.

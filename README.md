...
| After dotfiles | `run_after_install-freecad` | FreeCAD [latest stable](https://www.freecad.org/downloads.php) Linux AppImage from [GitHub Releases](https://github.com/FreeCAD/FreeCAD/releases) → `~/.local/opt/freecad` with `freecad` in `~/.local/bin`; installs [freecad-mcp](https://github.com/neka-nat/freecad-mcp) workbench + Cursor MCP (`uvx freecad-mcp`) with RPC auto-start (when `freecad` enabled; Linux only) |
...
| After dotfiles | `run_after_install-anki` | Latest official [Anki Linux .tar.zst](https://docs.ankiweb.net/platform/linux/installing.html) from [GitHub releases](https://github.com/ankitects/anki/releases) on every apply; upgrades when behind → `sudo ./install.sh` into `/usr/local` (when `anki` enabled) |
...
- **Externals / plugins:** fzf-tab and zsh-abbr install via `run_onchange_after_install-tools` (not chezmoi externals). fzf-tab must be the last OMZ plugin (see [Aloxaf/fzf-tab](https://github.com/Aloxaf/fzf-tab)); keep `omz_fzf` off when using fzf-tab. zsh-abbr v6+ needs the `zsh-job-queue` git submodule — if you see "problem finishing installing dependencies", run `chezmoi apply` (re-inits submodules) or `rm -rf ~/zsh-abbr && chezmoi apply`. User abbreviations are tracked at `~/.config/zsh-abbr/user-abbreviations` (chezmoi: `home/dot_config/zsh-abbr/user-abbreviations`). See [zsh-abbr#156](https://github.com/olets/zsh-abbr/issues/156).
...
- **Neovim / Telescope:** `telescope-fzf-native` must be **compiled** after install (`make` in the plugin dir). Termux needs `clang` and `make` (in the pkg list). The PackChanged hook runs `make` on install/update; if `libfzf.so` is still missing, run `make` in `~/.local/share/nvim/site/pack/core/opt/telescope-fzf-native`. Telescope falls back to the built-in sorter if the native library is absent.
- **Neovim / clipboard:** System clipboard uses `termux-clipboard-set` / `termux-clipboard-get` from the `termux-api` pkg (included in the Termux package list). Install the [Termux:API](https://github.com/termux/termux-api) Android app and grant it permission — without the app, `:checkhealth` will warn that no clipboard tool was found. Test with `termux-clipboard-set hi && termux-clipboard-get`. Shell bindings (`~/.bindings`) and `clip_copy` / `notify_user` in `~/.export` use the same API; notifications use `termux-toast` instead of `notify-send`.
...
- **Local LLM (llama.cpp):** Linux installs `llama-server` + `llama-swap` via `run_onchange_after_install-llama.sh.tmpl` and downloads pinned Hugging Face GGUFs to `~/.local/share/llama.cpp/models/`. The OpenAI-compatible API listens on `http://127.0.0.1:9292` (`coder` for Minuet FIM completions, `chat` for Avante). Enable with `llama: true` in profile. Termux uses `pkg install llama-cpp` and a runit `llama-server` service on the same port (no llama-swap). Logs: `$PREFIX/var/log/sv/llama-server/current`. If `sv up llama-server` warns about `supervise/ok`, run `source $PREFIX/etc/profile.d/start-services.sh`, wait a few seconds, then `sv-enable llama-server && sv up llama-server` again. **Migrating from Ollama:** after `chezmoi apply`, disable the old service (`sudo systemctl disable --now ollama`) and remove `~/.ollama` manually if you want the disk back (~15GB).
- **aichat:** With `llama: true`, `run_onchange_after_install-tools` installs [aichat](https://github.com/sigoden/aichat) via `cargo install`, [argc](https://github.com/sigoden/argc) + [llm-functions](https://github.com/sigoden/llm-functions), and a `~/.local/bin/aichat` wrapper. Bare interactive `aichat` opens an fzf session picker (`+ New session` or resume); any args / pipes pass through to the real binary (`~/.cargo/bin/aichat`). Tools (no API keys): DuckDuckGo `web_search`, `locate_binary`, `command_help`, `man_page`. Config: `~/.config/aichat/config.yaml` → llama-swap (`model: llama-swap:chat`, also `coder`). Legacy Textual `prompt`/`~/prompt` is no longer installed; old history under `~/.local/share/prompt/` is left alone if present.

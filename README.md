...
| After dotfiles | `run_after_install-freecad` | FreeCAD [latest stable](https://www.freecad.org/downloads.php) Linux AppImage from [GitHub Releases](https://github.com/FreeCAD/FreeCAD/releases) → `~/.local/opt/freecad` with `freecad` in `~/.local/bin` (when `freecad` enabled; Linux only) |

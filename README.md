...
| After dotfiles | `run_onchange_after_install-tools` | mise (Termux `pkg` on Android, `mise.run` on Linux), fzf-tab (OMZ plugin), zsh-abbr (v6.3.3), trash-cli (git + venv), ask CLI Textual venv when `llama` enabled, vivid (`.deb` on Linux / `pkg` on Termux); Linux also rust/zoxide via cargo when mise enabled |
...
- **ask CLI:** With `llama: true`, `run_onchange_after_install-tools` installs a Textual-based `ask` into `~/ask/.venv` (`ask` on `PATH` via `~/.local/bin/ask`). It queries the local `chat` model with allowlisted tools (`locate_binary`, `man`, `--help`) and DuckDuckGo when helpful (disable with `ask --no-web`). Q&A history is stored at `~/.local/share/ask/history.jsonl` (`ask --history`, `ask --history-clear`, `ask --no-history`). Use `ask --plain` in pipes/scripts. Trace a stuck run with `ask --debug` (JSONL at `~/.cache/ask/debug.log`, or `--debug-log PATH`). Example: `ask "how do I restore from trash-cli?"`.

...
- **Externals / plugins:** fzf-tab and zsh-abbr install via `run_onchange_after_install-tools` (not chezmoi externals). zsh-abbr v6+ needs the `zsh-job-queue` git submodule — if you see "problem finishing installing dependencies", run `chezmoi apply` (re-inits submodules) or `rm -rf ~/zsh-abbr && chezmoi apply`. User abbreviations are tracked at `~/.config/zsh-abbr/user-abbreviations` (chezmoi: `home/dot_config/zsh-abbr/user-abbreviations`). See [zsh-abbr#156](https://github.com/olets/zsh-abbr/issues/156).

...
| After dotfiles | `run_onchange_after_install-tools` | mise (Termux `pkg` on Android, `mise.run` on Linux), fzf-tab (OMZ plugin), zsh-abbr (v6.3.3), trash-cli (git + venv), aichat + llm-functions (DuckDuckGo `web_search`) when `llama` enabled, vivid (`.deb` on Linux / `pkg` on Termux); Linux also rust/zoxide via cargo when mise enabled |
...
- **aichat:** With `llama: true`, `run_onchange_after_install-tools` installs [aichat](https://github.com/sigoden/aichat) via `cargo install`, [argc](https://github.com/sigoden/argc) + [llm-functions](https://github.com/sigoden/llm-functions) (DuckDuckGo `web_search`, no API key), and a `~/.local/bin/aichat` wrapper. Bare interactive `aichat` opens an fzf session picker (`+ New session` or resume); any args / pipes pass through to the real binary (`~/.cargo/bin/aichat`). Config: `~/.config/aichat/config.yaml` → llama-swap (`model: llama-swap:chat`, also `coder`; `use_tools: web_search`). Legacy Textual `prompt`/`~/prompt` is no longer installed; old history under `~/.local/share/prompt/` is left alone if present.

...
- **ask CLI:** With `llama: true`, `run_onchange_after_install-tools` installs a Textual-based `ask` into `~/ask/.venv` (`ask` on `PATH` via `~/.local/bin/ask`). It queries the local `chat` model with allowlisted tools (`locate_binary`, `man`, `--help`) and DuckDuckGo when helpful (disable with `ask --no-web`). Use `ask --plain` in pipes/scripts. Trace a stuck run with `ask --debug` (JSONL at `~/.cache/ask/debug.log`, or `--debug-log PATH`). Example: `ask "how do I restore from trash-cli?"`.

...
| After dotfiles | `run_onchange_after_install-tools` | mise (Termux `pkg` on Android, `mise.run` on Linux), fzf-tab (OMZ plugin), zsh-abbr (v6.3.3), trash-cli (git + venv), prompt CLI Textual venv when `llama` enabled, vivid (`.deb` on Linux / `pkg` on Termux); Linux also rust/zoxide via cargo when mise enabled |
...
- **prompt CLI:** With `llama: true`, `run_onchange_after_install-tools` installs a Textual-based `prompt` into `~/prompt/.venv` (`prompt` on `PATH` via `~/.local/bin/prompt`). It queries the local `chat` model with allowlisted tools (`locate_binary`, `man`, `--help`) and DuckDuckGo when helpful (disable with `prompt --no-web`). Q&A history is stored at `~/.local/share/prompt/history.jsonl` (`prompt --history`, `prompt --follow-up`, `prompt --history-clear`). Use `prompt --plain` in pipes/scripts. Trace a stuck run with `prompt --debug` (JSONL at `~/.cache/prompt/debug.log`, or `--debug-log PATH`). Example: `prompt "how do I restore from trash-cli?"`.
- **aichat:** Same `llama` feature installs [aichat](https://github.com/sigoden/aichat) via `cargo install` and deploys `~/.config/aichat/config.yaml` pointed at llama-swap (`model: llama-swap:chat`, also `coder`). Use `aichat` for a terminal Chat-REPL with sessions (`.session`); use `prompt` when you want the tool-using agent.
